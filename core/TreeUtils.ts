 ...
 * 树节点绑定数据的类型
 */
export type NodeDataType = JChar | JString | JInt | JLong | JFloat | JDouble | JBigDecimal | JBoolean | JDate | JSqlDate | JSqlTime | JSqlTimestamp | Date | number | string | boolean | null | undefined;
...
export interface TreeNodeRes<IDT> {
    /** 节点ID */
    getId(): IDT;
...
export interface StringTreeNodeRes extends TreeNodeRes<JString> {
    /** 返回所有子节点 */
    getChildren(): JList<StringTreeNodeRes>;
}

export interface LongTreeNodeRes extends TreeNodeRes<JString> {
    /** 返回所有子节点 */
...
const treeUtils: TreeUtils = Java.type('org.clever.hinny.graal.core.TreeUtils').Instance;
